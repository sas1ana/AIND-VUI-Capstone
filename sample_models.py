...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    for layer in range(recur_layers):
        ...
        rnn = GRU(units, activation='relu',
                  ...
                  name=rnn_name)(block)
        block = BatchNormalization(name=bn_rnn_name)(rnn)
        
    # TODO: Add a TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim))(block)
...
def final_model(input_dim, filters, kernel_size, conv_stride,
    ...
    dpout1 = Dropout(0.25)(bn_cnn)
    bidir_rnn = Bidirectional(LSTM(units, return_sequences=True, name="bidirectional", implementation=2))(dpout1)
    bn_bidir_rnn = BatchNormalization()(bidir_rnn)
    dpout2 = Dropout(0.3)(bn_bidir_rnn)
    
    #TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim))(dpout2)
